...
app.layout = html.Div(
    [
        ...
        html.Div([
            ...
            html.Img(src="https://miro.medium.com/v2/resize:fit:4800/format:webp/1*_SRbz8535-CxPzo6IL9pBA.png",
...
def plot_piechart(x, color, components, observations):
    ...
    if color == "yes":
        ...
        fig.add_trace(go.Pie(labels=temp_1.index, values=temp_1["count"], pull=explode_1, name="reordered"), row=1,
                      col=1)
        fig.add_trace(go.Pie(labels=temp_2.index, values=temp_2["count"], pull=explode_2, name="not reordered"), row=1,
                      col=2)
        fig.update_layout(
    ...
    for i, var1 in enumerate(temp_df.columns):
        for j, var2 in enumerate(temp_df.columns):
            ...
            if i == j:  # Diagonal - histogram
                ...
                fig.add_trace(go.Scatter(x=temp_df[var1], y=temp_df[var2], marker=color, mode='markers',
                                         name=f"scatter_{var1}_{var2}"), row=i + 1, col=j + 1)
    ...
    if isinstance(x, str):
        ...
        for idx in x:
            if binwidth is not None:
                fig = go.Figure(
                    data=go.Violin(y=temp_df[idx], box_visible=True, meanline_visible=True, opacity=binwidth, name=idx))
            else:
...
def plot_heatmap(x, observations):
    ...
    # Create a box plot
    fig = px.imshow(df_corr, labels=dict(color="Correlation Coefficients"), x=df_corr.columns, y=df_corr.columns,
                    zmin=-0.5, zmax=0.6, color_continuous_scale="RdBu_r")
    annot = []
